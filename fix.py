...
start_marker = "function generateTechSheetPdf(): Buffer {"
end_marker = "export async function POST(request: NextRequest) {"
...
new_fn = '''function generateTechSheetPdf(): Buffer {
  ...
  const layerLabel = (label: string, ry: number) => {
    ...
    doc.text(label, lx, ry + bh / 2, { baseline: 'middle' })
  }
  ...
  const arrowH = (x1: number, x2: number, ay: number, lines: string[]) => {
    doc.setDrawColor(156, 163, 175); doc.setLineWidth(0.3)
    doc.line(x1, ay, x2 - 1, ay)
    doc.setFillColor(156, 163, 175)
    doc.triangle(x2, ay, x2 - 1.5, ay - 0.8, x2 - 1.5, ay + 0.8, 'F')
    if (lines.length > 0) {
      doc.setFont('helvetica', 'normal'); doc.setFontSize(5.5)
      const midX = (x1 + x2) / 2
      const maxW = lines.reduce((m: number, l: string) => Math.max(m, doc.getTextWidth(l)), 0)
      const bgH = lines.length * 3.5 + 1
      doc.setFillColor(255, 255, 255)
      doc.rect(midX - maxW / 2 - 0.5, ay + 0.8, maxW + 1, bgH, 'F')
      doc.setTextColor(156, 163, 175)
      lines.forEach((line: string, i: number) => {
        doc.text(line, midX, ay + 2.5 + i * 3.5, { align: 'center' })
      })
    }
  ...
  const arrowV = (ax: number, y1: number, y2: number, label: string | null) => {
    ...
    doc.setFillColor(156, 163, 175)
    doc.triangle(ax, y2, ax - 0.8, y2 - 1.5, ax + 0.8, y2 - 1.5, 'F')
    if (label) {
      doc.setFont('helvetica', 'normal'); doc.setFontSize(5.5)
      const midY = (y1 + y2) / 2
      const tw = doc.getTextWidth(label)
      doc.setFillColor(255, 255, 255)
      doc.rect(ax + 1, midY - 2, tw + 1, 4, 'F')
      doc.setTextColor(156, 163, 175)
      doc.text(label, ax + 1.5, midY, { baseline: 'middle' })
    }
    doc.setLineWidth(0.2)
  }
  ...
  const arrowLeft = (x1: number, x2: number, ay: number, label: string | null) => {
    ...
    if (label) {
      ...
      const tw = doc.getTextWidth(label)
      doc.setFillColor(255, 255, 255)
      doc.rect(midX - tw / 2 - 0.5, ay + 0.8, tw + 1, 4, 'F')
      doc.setTextColor(156, 163, 175)
      doc.text(label, midX, ay + 2.8, { align: 'center' })
    }
    doc.setLineWidth(0.2)
  }
  ...
  arrowV(c1x + c1w / 2, r1y + bh, r2y, 'access code + PDF')
  ...
  doc.setFillColor(156, 163, 175)
  doc.triangle(c3x, r4y + bh / 2, c3x - 1.5, r4y + bh / 2 - 0.8, c3x - 1.5, r4y + bh / 2 + 0.8, 'F')
  doc.setLineWidth(0.2)
