...
# ─────────────────────────────────────────────
# FICHEIRO 1: solicitar/route.ts  (email do código)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# FICHEIRO 2: analisar/route.ts  (PDF + emails)
# ─────────────────────────────────────────────
...
function generateAnalysisPdf(analysisText: string, generatedDate: string): Buffer {
  ...
  const drawFooter = () => {
    ...
    for (let p = 1; p <= total; p++) {
      ...
      if (p === total) {
        ...
        const fullLine = prefix + linkLabel
        ...
        doc.textWithLink(linkLabel, linkX, builtY, { url: `${appUrl}/privacy-policy` })
      }
    }
  }
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith('- ')) {
      ...
      // Flag [!] — só "[!] Alert:" a laranja, resto cinzento
      if (text.startsWith('[!]')) {
        ...
        // Linha 1: "[!] Alert:" em laranja, resto em cinzento
        doc.setFontSize(8.5)
        ...
        // O resto da primeira linha
        const firstLineRest = wrapped[0].replace(alertLabel + '  ', '')
        doc.setFont('helvetica', 'normal')
        doc.setTextColor(55, 65, 81)
        doc.text(firstLineRest, margin + 5 + labelWidth, y)

        // Linhas seguintes completamente em cinzento
        for (let wl = 1; wl < wrapped.length; wl++) {
          y += 4.5
          doc.text(wrapped[wl], margin + 5, y)
        }
        y += 5
        continue
      }

      // Bullet normal — remove asteriscos
      doc.setFontSize(8.5)
      doc.setTextColor(55, 65, 81)
      doc.text('\u2022', margin, y)
      ...
      // Primeira parte em bold se havia **texto:**
      if (text.includes('**')) {
    ...
    if (line.startsWith('**')) {
      ...
      if (plain.length > 1) {
        for (let wl = 1; wl < plain.length; wl++) {
          y += 5
          doc.setFont('helvetica', 'normal')
          doc.text(plain[wl], margin, y)
        }
      }
      y += 5.5
      continue
...
function generateTechSheetPdf(): Buffer {
  ...
  doc.setFontSize(8.5)
  doc.setTextColor(107, 114, 128)
  doc.text('Technical Overview  \u00b7  Built by Marco Costa', pageWidth / 2, y, { align: 'center' })
  y += 7
  ...
  const item = (label: string, detail: string) => {
    ...
    doc.text(detailLines, margin + colLabel, y)
    y += detailLines.length * 4.5 + 1.5
  }
  ...
  const features = [
    ...
    'Automated data retention policy: analyses deleted after 3 days, codes after 30 days',
  ...
  doc.setFontSize(7.5)
  doc.setTextColor(156, 163, 175)
  doc.text('Built by Marco Costa  \u00b7  Legal Engineer  \u00b7  For portfolio demonstration purposes', pageWidth / 2, y, { align: 'center' })
  y += 4
  doc.setTextColor(29, 78, 216)
...
- **[Issue]:** [specific recommended clause language or action]`
    ...
    // Guarda contrato no Storage
    const fileName = `${code}-${Date.now()}.pdf`
    await supabaseAdmin.storage
      .from('contracts')
      .upload(fileName, buffer, { contentType: 'application/pdf', upsert: false })

    // Actualiza código de acesso
    await supabaseAdmin
      .from('access_codes')
    ...
    // Email ao utilizador — análise + ficha técnica
    if (email) {
      await resend.emails.send({
        from: 'Contract Analyser <contractanalyzer@lawper.pt>',
        to: email,
        ...
        attachments: [
          {
            ...
            content: analysisPdfBuffer.toString('base64'),
          },
          {
            filename: 'contract-analyser-tech-overview.pdf',
            content: techSheetBuffer.toString('base64'),
          },
        ],
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 48px 24px;">
            ...
            <p style="font-size: 14px; color: #374151; margin: 0 0 16px;">
              Please find your contract analysis report attached to this email as a PDF.
            </p>

            <p style="font-size: 14px; color: #374151; margin: 0 0 32px;">
              I have also included a one-page technical overview of the application, should you find it relevant.
            </p>

            <p style="font-size: 12px; color: #6b7280; border-top: 1px solid #f3f4f6; padding-top: 20px; margin: 0 0 16px;">
              <strong>Disclaimer:</strong> This analysis is generated by AI for informational purposes only.
