...
function generateAnalysisPdf(analysisText: string, generatedDate: string): Buffer {
  ...
  // Clean text — replace emoji warning with [!]
  ...
  const drawFooter = () => {
    ...
    for (let p = 1; p <= totalPages; p++) {
      doc.setPage(p)
      if (p === totalPages) {
        // Last page: disclaimer + built by centered
        doc.setDrawColor(229, 231, 235)
        doc.line(margin, footerY - 8, pageWidth - rightMargin, footerY - 8)
        doc.setFont('helvetica', 'bold')
        doc.setFontSize(7.5)
        doc.setTextColor(156, 163, 175)
        doc.text('Disclaimer:', margin, footerY - 4)
        ...
        const dLines = doc.splitTextToSize(disclaimer, contentWidth - 22)
        doc.text(dLines, margin + 22, footerY - 4)
        const builtY = footerY - 4 + dLines.length * 3.5 + 3
        doc.setTextColor(156, 163, 175)
        doc.text('Built by Marco Costa  \u00b7  For portfolio demonstration purposes  \u00b7  ', pageWidth / 2, builtY, { align: 'center' })
        const linkText = 'Privacy Policy'
        const builtBase = 'Built by Marco Costa  \u00b7  For portfolio demonstration purposes  \u00b7  '
        const builtWidth = doc.getTextWidth(builtBase)
        const linkX = pageWidth / 2 - builtWidth / 2 + builtWidth - doc.getTextWidth(linkText) / 2
        doc.setTextColor(29, 78, 216)
        doc.textWithLink(linkText, pageWidth / 2 + doc.getTextWidth(builtBase) / 2 - doc.getTextWidth(linkText) / 2, builtY, { url: `${appUrl}/privacy-policy` })
      }
    }
  }
  ...
  doc.setDrawColor(229, 231, 235)
  doc.line(margin, y, pageWidth - rightMargin, y)
  y += 8
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('- ')) {
      ...
      if (text.startsWith('[!]')) {
        ...
        if (text.includes('**')) {
          // Render first line with bold parts, rest normal
    ...
    // Line starting with **bold:** rest normal
    if (line.startsWith('**')) {
      checkPageBreak(7)
      doc.setFontSize(9)
      doc.setTextColor(17, 24, 39)
      ...
      let x = margin
      doc.setFont('helvetica', 'normal')
      for (let p = 0; p < parts.length && p < 4; p++) {
        if (!parts[p]) continue
        doc.setFont('helvetica', p % 2 === 1 ? 'bold' : 'normal')
        doc.text(parts[p], x, y)
        x += doc.getTextWidth(parts[p])
        if (x > pageWidth - rightMargin - 5) break
      }
      if (wrapped.length > 1) {
        for (let wl = 1; wl < wrapped.length; wl++) {
          y += 5
          doc.setFont('helvetica', 'normal')
          doc.text(wrapped[wl], margin, y)
        }
      }
      y += 5.5
      continue
...
- **[Issue]:** [specific recommended clause language or action]`
    ...
    if (email) {
      await resend.emails.send({
        ...
        attachments: [{ filename: 'contract-analysis.pdf', content: pdfBase64 }],
        html: `
...
print("analisar/route.ts reescrito - PDF melhorado com [!], margens corrigidas, footer centrado.")
